    ...
    public String panelAdmin(Model model) {
        try {
            ...
            EstadisticasDashboard estadisticas = new EstadisticasDashboard(
                (int) usuarioService.count(),
                equipoService.findByEstado(Estado.ACTIVO).size(),
                ligaService.findAll().size(),
                partidoService.findPartidosEsteMes().size()
            );
            
            model.addAttribute("estadisticas", estadisticas);
            logger.info("Dashboard cargado exitosamente - Usuarios: {}, Equipos: {}, Ligas: {}, Partidos: {}", 
                estadisticas.getTotalUsuarios(), estadisticas.getEquiposActivos(), 
